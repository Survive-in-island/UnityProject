...
using UnityEngine;
using System;
public class TigerArea : Area
{
    [Header("Tiger Area Objects")]
    public GameObject tigerAgent;
    ...
    {
        // 돼지가 벗어나지 않게
        Vector3 pigLocalPosition = tigerAgent.transform.localPosition;
        if (Mathf.Abs(pigLocalPosition.x) > 13f || Mathf.Abs(pigLocalPosition.z) > 13f)
        {
            Debug.LogWarning("Pig out of the pen!");
            TigerAgent pigAgentComponent = tigerAgent.GetComponent<TigerAgent>();
    ...
    {
        // 위치와 회전값 리셋 
        RandomlyPlaceObject(tigerAgent, spawnRange, 10);
    }
